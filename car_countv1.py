#second go at this open cv car counting nonsense
#source: https://github.com/jorgem0/traffic_counter/blob/master/traffic_counter.py
#ONLY COUNTS CARS GOING DOWN, IF WE EVER NEED CARS GOING UP (2 FLOWS OF TRAFFIC) UNCOMMENT A SECTION

'''
notes section: right now it is counting people as well
(which will probably slow it down, see if it can JUST count cars
allow the contour drawings to show - figure out which is slowing what
right now it is counting many things as contours when not even close to the counting area
efficiecy: counting time vs. actual time (want it to be as close to 1.0), do for around a minute
=.46

eventually will want to do it for the whole video, every video

so right now video framing is 960 by 540, but can probably cut out a lot
because of the video angle (reduce computing power needed)
new note - we know the cars are flowing - instead of crossing a line
just count how many can cross - recognize the car and count it (ONCE
ONLY)
...
cap = cv2.VideoCapture('car1_film.mov')
...
while True:
    ...
    if ret:  # if there is a frame continue with code
        ...
        cv2.line(image, (60, lineypos), (450, lineypos), (255, 0, 0), 5)
        ...
        cv2.line(image, (60, lineypos2), (450, lineypos2), (0, 255, 0), 5)
...
#                   #this one draws another small rectange(still shows)
        ...
        #see if we don't need the crossed up - not counting that
        #not going to include it for now
